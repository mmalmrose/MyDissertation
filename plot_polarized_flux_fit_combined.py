...
def plot_polarized_flux_fit_combined( polarflux, spolarflux, polarnu, mjd, plotout,  restnu, nuspec, model, name, polspec):
    fig = plt.figure(8, figsize=(10,7)) 
    gs = gridspec.GridSpec(2,1, height_ratios=[3,1])
    ax1 = plt.subplot(gs[0])
    nunorm, fluxnorm, modelnorm = gom(polarnu), gom(polarflux), gom(model)
    ax1.errorbar(polarnu/nunorm, polarflux/fluxnorm, yerr =4.0*spolarflux/fluxnorm,  fmt='o', color = (0,1,1))
    ...
    ax1.set_ylabel(r'$F_{\mathrm{p,\nu\;}} (10^{'+str(np.trunc(np.log10(fluxnorm)).astype(int))+r'}\mathrm{erg\;s}^{-1} \mathrm{\;cm}^{-2} \mathrm{\;Hz}^{-1}\;)$').set_fontsize(15)
    ax1.set_title("MJD = "+str(format(mjd, '.2f'))).set_fontsize(18)
    ax2.errorbar(polarnu/nunorm, (polarflux-model)/(model), yerr=4.0*spolarflux/model,fmt='o', color = (0,1,1))
